...
from meta_bilstm.nn_utils.losses import seq_loss, calc_accuracy
from meta_bilstm.utils.dataset import create_dataloader, PosTagDataset
...
class Trainer:
    ...
    def __init__(self,
        ...
        for model_name in model_config:
            model_config[model_name]['output_proj_size'] = len(data["tags"]["tag_to_idx"])
            model_config[model_name]['device'] = device
            
        model_config["word_model_params"]['dataset'] = data['datasets']['train']
        model_config["char_model_params"]['num_embs'] = len(data['chars']['char_to_idx'])
        ...
        self.datasets = {
            "train": PosTagDataset(
                data['datasets']['train'], self.model.word_model.emb_layer.token_to_idx, data['chars']['char_to_idx']
            ),
            "test": PosTagDataset(
                data['datasets']['test'], self.model.word_model.emb_layer.token_to_idx, data['chars']['char_to_idx']
            )
        }
        self.dataloaders = {
    ...
    def train_model(self,
        ...
        schedulers = {
            "char": lr_scheduler.StepLR(optimizers["char"], step_size=scheduler_step_size, gamma=scheduler_gamma),
            "word": lr_scheduler.StepLR(optimizers["word"], step_size=scheduler_step_size, gamma=scheduler_gamma),
            "meta": lr_scheduler.StepLR(optimizers["meta"], step_size=scheduler_step_size, gamma=scheduler_gamma),
        }
    ...
    def validate_model(self, test_loader):
        ...
        for batch in tqdm(test_loader):
            ...
            with torch.no_grad():
                for model_name in self.model_names:
                    metrics['loss'][model_name].append(seq_loss(
                        outputs[model_name]["logits"],
                        outputs[model_name]['lens'],
                        batch['labels'].to(self.device)
                    ))

                    metrics['acc'][model_name].append(calc_accuracy(
                        batch['word_idx'][0].to(self.device),
                        outputs[model_name]['logits'],
                        batch['labels'].to(self.device)
                    ))
        for metric in metrics:
            for model_name in self.model_names:
                metrics[metric][model_name] = sum(metrics[metric][model_name]) / len(sum(metrics[metric][model_name]))
        return metrics
